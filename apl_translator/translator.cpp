...
token_t token( int line, int pos, std::string apl = "", std::string value = "", double valuef = 0 )
{
    token_t tk;
    tk.line = line;
    tk.pos = pos;
    ...
    tk.valuef = valuef;
    tk.suppress = 0;
    //printf("token(%d, %d, %s, %s, %f)\n", line, pos, apl.c_str(), value.c_str(), valuef);
    //while(getchar()!='\n');
    return tk;
}
...
void veprintf( token_t tok, int type, const char* message, va_list vl )
{
    char* p;
    char buffer[1024] = {0};
    int pos = tok.pos;
...
void eprintf( token_t tok, int type, const char* message, ... )
{
    va_list vl;
    va_start( vl, message );
...
int lookup_act( std::string name )
{
...
int lookup_punc( std::string name )
{
...
double read_const( std::string name )
{
    double value = 0;
    int i;
...
std::map<std::string, std::vector<token_t> > simc_apls;
...
token_t NAL = token( -1, -1, "" );
std::stack<int> mark_stack;
int pos;
int last;
std::vector<token_t>* working;
...
std::map<std::string, apl_t> ic_apls;

cond_node_t* constant()
{
    token_t& t = get();
    if( t.value.empty() )
    {
...
cond_node_t* logical_expression();
cond_node_t* primary_expression()
{
    cond_node_t* child;
    mark();
    if( 0 != ( child = variable() ) )
...
cond_node_t* multiplicative_expression()
{
    cond_node_t* lhs = 0;
    cond_node_t* rhs = 0;
    std::string op;
...
cond_node_t* additive_expression()
{
    cond_node_t* lhs = 0;
    cond_node_t* rhs = 0;
    std::string op;
...
cond_node_t* relational_expression()
{
    cond_node_t* lhs = 0;
    cond_node_t* rhs = 0;
    std::string op;
...
int kv_pair( std::string& key, int& vpos )
{
    std::string tkey;
    int tvpos;
    if( get().value == "," )
...
int main( int argc, char** argv )
{
    if ( argc < 2 )
    {
        printf( "IreCore APL Translator (from SimC)\n"
                "Aeandarian 2015/8/21\n"
                "Usage: translator <simc-profile> <output> [-p]\n"
                "\t-p : prepend \"actions+=\" each line." );
        return 0;
    }
    if ( argc >= 4 ) {
        if ( strcmp( argv[3], "-p" ) == 0 ) prepend = 1;
    }
    read_source( argv[1] );
    lexer();
    parser();
    ...
    fclose( f );
    printf( "translation complete, %d errors, %d warnings", errors, warnings );
    return 0;
}
